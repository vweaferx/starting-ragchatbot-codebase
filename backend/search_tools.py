...
class CourseSearchTool(Tool):
    ...
    def _format_results(self, results: SearchResults) -> str:
        ...
        for doc, meta in zip(results.documents, results.metadata):
            ...
            # Look up lesson link and wrap in anchor tag if available
            link = None
            if lesson_num is not None:
                link = self.store.get_lesson_link(course_title, lesson_num)
            if link:
                source = f'<a href="{link}" target="_blank" rel="noopener noreferrer">{label}</a>'
            else:
                source = label
        ...
        return {
            ...
            "input_schema": {
                ...
                "properties": {
                    "course_name": {
                        "type": "string",
                        "description": "Course title or partial name (e.g. 'MCP', 'RAG', 'Chroma')"
                    }
                },
